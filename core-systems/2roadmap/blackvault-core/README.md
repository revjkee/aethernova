# Blackvault Core Core System

Blackvault Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность blackvault core.
...
```python
from blackvault_core.config import config
...
### Программный запуск
```python
from blackvault_core import BlackvaultcoreCore
...
# Запуск с покрытием
pytest tests/ --cov=blackvault_core
```
