# Silentlink Core Core System

Silentlink Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность silentlink core.
...
```python
from silentlink_core.config import config
...
### Программный запуск
```python
from silentlink_core import SilentlinkcoreCore
...
# Запуск с покрытием
pytest tests/ --cov=silentlink_core
```
