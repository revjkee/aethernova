# Genesisops Core Core System

Genesisops Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность genesisops core.
...
```python
from genesisops_core.config import config
...
### Программный запуск
```python
from genesisops_core import GenesisopscoreCore
...
# Запуск с покрытием
pytest tests/ --cov=genesisops_core
```
