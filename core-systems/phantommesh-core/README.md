# Phantommesh Core Core System

Phantommesh Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность phantommesh core.
...
```python
from phantommesh_core.config import config
...
### Программный запуск
```python
from phantommesh_core import PhantommeshcoreCore
...
# Запуск с покрытием
pytest tests/ --cov=phantommesh_core
```
