# Identity Access Core Core System

Identity Access Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность identity access core.
...
```python
from identity_access_core.config import config
...
### Программный запуск
```python
from identity_access_core import IdentityaccesscoreCore
...
# Запуск с покрытием
pytest tests/ --cov=identity_access_core
```
