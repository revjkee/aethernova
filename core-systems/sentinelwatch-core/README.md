# Sentinelwatch Core Core System

Sentinelwatch Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность sentinelwatch core.
...
```python
from sentinelwatch_core.config import config
...
### Программный запуск
```python
from sentinelwatch_core import SentinelwatchcoreCore
...
# Запуск с покрытием
pytest tests/ --cov=sentinelwatch_core
```
