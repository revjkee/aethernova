...
class OpenTelemetryFormatter(logging.Formatter):
    ...
    def __init__(self, service_name: str | None = None, use_json: bool = True):
        super().__init__()
        self.service_name = service_name or "observability-core"
    ...
    def format(self, record: logging.LogRecord) -> str:
        try:
            ...
            raise RuntimeError(
                "OpenTelemetryFormatter requires `pip install .[integrations]`"
            ) from exc
        ...
        span_id = (
            format(span_context.span_id, "016x") if span_context and span_context.span_id else None
        )
