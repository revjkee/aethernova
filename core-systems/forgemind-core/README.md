# Forgemind Core Core System

Forgemind Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность forgemind core.
...
```python
from forgemind_core.config import config
...
### Программный запуск
```python
from forgemind_core import ForgemindcoreCore
...
# Запуск с покрытием
pytest tests/ --cov=forgemind_core
```
