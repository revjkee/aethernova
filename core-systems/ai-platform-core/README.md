# Ai Platform Core Core System

Ai Platform Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность ai platform core.
...
```python
from ai_platform_core.config import config
...
### Программный запуск
```python
from ai_platform_core import AiplatformcoreCore
...
# Запуск с покрытием
pytest tests/ --cov=ai_platform_core
```
