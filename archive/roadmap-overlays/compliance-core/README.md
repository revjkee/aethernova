# Compliance Core Core System

Compliance Core - это ключевой компонент экосистемы AetherNova, предоставляющий функциональность compliance core.
...
```python
from compliance_core.config import config
...
### Программный запуск
```python
from compliance_core import CompliancecoreCore
...
# Запуск с покрытием
pytest tests/ --cov=compliance_core
```
