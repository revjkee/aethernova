...
class StrategyRouter:
    ...
    async def route(self, message: AgentMessage):
        ...
        """
        # Добавляем сообщение в приоритетную очередь
        await self.priority_queue.put(message)
        logger.debug(f"Message {message.message_id} added to priority queue (priority: {message.priority})")
    
    async def _process_queue(self):
        """Фоновый процесс обработки приоритетной очереди"""
        while True:
            try:
    ...
    async def _route_message(self, message: AgentMessage):
        """Маршрутизация конкретного сообщения"""
        try:
    ...
    async def _route_smart(self, message: AgentMessage) -> Optional[str]:
        ...
        complexity = self._analyze_task_complexity(payload)
    ...
    def _analyze_task_complexity(self, payload: Dict) -> float:
        """Анализ сложности задачи для выбора подходящего типа агента"""
        ...
        complex_keywords = ['optimize', 'learn', 'adapt', 'analyze', 'complex', 'multi-step']
        simple_keywords = ['filter', 'validate', 'check', 'simple', 'basic']
    ...
    def update_agent_metrics(self, agent_id: str, current_load: int, max_capacity: int, 
                           avg_response_time: float, success_rate: float):
        """Обновить метрики агента для балансировки нагрузки"""
        ...
        self.load_balancer.update_agent_load(agent_id, load_info)
        logger.debug(f"Updated metrics for agent {agent_id}: load={current_load}/{max_capacity}, "
                    f"response_time={avg_response_time}ms, success_rate={success_rate:.2%}")
    
    def get_routing_stats(self) -> Dict[str, Any]:
        """Получить статистику маршрутизации"""
        return {
            "task_type_distribution": dict(self.routing_stats),
            "total_routed_messages": sum(self.routing_stats.values()),
            "queue_size": self.priority_queue.size(),
            "active_agents": len(self.load_balancer.agent_loads),
            "agent_loads": {
                agent_id: {
                    "load_ratio": info.load_ratio,
                    "efficiency_score": info.efficiency_score,
                    "current_load": info.current_load,
                    "success_rate": info.success_rate
                }
                for agent_id, info in self.load_balancer.agent_loads.items()
            }
        }
    
    def get_agent_recommendations(self) -> Dict[str, str]:
        """Получить рекомендации по оптимизации агентов"""
        recommendations = {}
        
        for agent_id, load_info in self.load_balancer.agent_loads.items():
            if load_info.load_ratio > 0.9:
                recommendations[agent_id] = "Высокая загруженность - рассмотрите масштабирование"
            elif load_info.success_rate < 0.7:
                recommendations[agent_id] = "Низкая успешность - требуется диагностика"
            elif load_info.avg_response_time > 5000:
                recommendations[agent_id] = "Медленный отклик - оптимизируйте производительность"
            elif load_info.load_ratio < 0.2:
                recommendations[agent_id] = "Низкая загруженность - можно перераспределить задачи"
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy" if not self.priority_queue.empty() else "idle",
            "queue_size": self.priority_queue.size(),
            "registered_agents": len(self.registry.list_agents()),
            "active_load_balancer": len(self.load_balancer.agent_loads) > 0,
            "supported_task_types": list(self.strategy_map.keys()),
            "uptime": time.time()  # можно добавить реальный uptime
        }
    
    def report_task_completion(self, agent_id: str, message_id: str, success: bool, 
                             response_time_ms: float):
        """Сообщить о завершении задачи для обновления метрик"""
        # Уменьшить текущую нагрузку агента
        if agent_id in self.load_balancer.agent_loads:
            self.load_balancer.agent_loads[agent_id].current_load = max(
                0, self.load_balancer.agent_loads[agent_id].current_load - 1
            )
        
        # Записать в историю для анализа
        self.load_balancer.record_task_completion(agent_id, success, response_time_ms)
        
        logger.debug(f"Task completion reported: agent={agent_id}, message={message_id}, "
