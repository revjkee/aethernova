...
function buildCsv(data: ReturnType<typeof normalizeData>, unit: YUnit): string {
  ...
  const rows = data.map((d) => {
    ...
    return [
      ...
      typeof errorsPercent === "number" ? errorsPercent.toFixed(3) : errorsPercent,
    ].join(",");
  });
          ...
          <div className="flex flex-wrap items-center gap-3 justify-start lg:justify-end">
            <div className="flex items-center gap-2">
              ...
              <Input
                ...
                className="h-8 w-28"
                value={warn ?? ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setWarn(parseNum(e.target.value))}
              />
            </div>
            <div className="flex items-center gap-2">
              ...
              <Input
                ...
                className="h-8 w-28"
                value={crit ?? ""}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCrit(parseNum(e.target.value))}
              />
            </div>
          </div>
        </div>
