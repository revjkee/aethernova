...
const VoteIntegrityStatus: React.FC<VoteIntegrityStatusProps> = ({ proposalId }) => {
  ...
  const scoreClass = integrityScore > 85 ? 'text-green-600' : integrityScore > 60 ? 'text-yellow-600' : 'text-red-600'
      ...
      <CardContent className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            {integrityScore >= 85 ? <ShieldCheck className="text-green-500" size={24} /> : <AlertTriangle className="text-yellow-500" size={24} />}
        ...
        <div className="text-sm text-muted-foreground leading-relaxed">{aiSummary}</div>
          ...
          <div className="flex items-center gap-2">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>Количество кошельков, не прошедших ZK-проверку.</TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ZK-провалы: <span className="font-medium">{zkFailureCount}</span>
          </div>

          <div className="flex items-center gap-2">Sybil-кластеры: <span className="font-medium">{sybilClusters}</span></div>
          <div className="flex items-center gap-2">Ботов голосовало (оценка AI): <span className="font-medium">{botVoteEstimate}%</span></div>
          <div className="flex items-center gap-2">Флагнутые кошельки: <span className="font-medium">{suspiciousWallets.length}</span></div>
        </div>
